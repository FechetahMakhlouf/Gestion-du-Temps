...
def get_timeslots():
    ...
    return json_response([{'id': t.id, 'start': t.start, 'end': t.end} for t in ts])
...
def add_timeslot():
    ...
    for t in existing:
        if not (end <= t.start or start >= t.end):
            return json_response(message='Overlaps with existing timeslot', status=400)

    ts_id = f"ts_{int(time.time()*1000)}"
    ts = Timeslot(id=ts_id, user_id=current_user.id, start=start, end=end)
    db.session.add(ts)
    db.session.commit()
